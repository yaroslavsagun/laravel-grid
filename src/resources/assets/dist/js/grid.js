...
(function ($) {
    ...
    (function ($) {
        ...
        _grids.utils.handleAjaxRequest = function (element, event, options) {
            ...
            element.each(function (i, obj) {
                ...
                let data = {};
                let bulkAction = obj.data('bulk-action');
                ...
                obj.on(event, function (e) {
                    ...
                    if (bulkAction) {
                        let checked = [];
                        $(".selection input:checkbox:checked").each(function () {
                    ...
                    $.ajax({
                        ...
                        data: isForm ? obj.serialize() : data,
        ...
        var grid = function () {
            ...
            _createClass(grid, [{
                ...
                value: function setupPjax(container, target, afterPjax, options) {
                    ...
                    // global timeout
                    $.pjax.defaults.timeout = options.timeout || 20000;
    ...
    _grids.init = function () {
        ...
        if (typeof daterangepicker !== 'function') {
            ...
            //bulk actions
            $('.laravel-grid').on('change', '.selection input:checkbox', function (e) {
                if ($(".selection input:checkbox:checked").length) {
                    $('#grid-delete-selected').removeClass('disabled');
                } else {
                    $('#grid-delete-selected').addClass('disabled');
                }

                if ($(".selection input:checkbox:not(:checked)").length) {
                    if ($('#grid-select-all').hasClass('btn-primary')) {
                        $('#grid-select-all').toggleClass('btn-primary btn-success');
                        $('#grid-select-all').html("<i class='fa fa-square'></i> Select All");
                    }
                } else {
                    $('#grid-select-all').toggleClass('btn-success btn-primary');
                    $('#grid-select-all').html("<i class='fa fa-check-square'></i> Unselect All");
                }
            });

            $('#grid-select-all').on('click', function (e) {
                let $notCheckedList = $(".selection input:checkbox:not(:checked)");
                if ($notCheckedList.length) {
                    $notCheckedList.each(function () {
